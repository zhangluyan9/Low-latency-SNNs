...
class spikeLayer(torch.nn.Module):
    ...
    def generate_spike(self,theta=0.9999,T=5):
        return _generate_spike(theta,T)
...
class _poolLayer(nn.Conv3d):
    ...
    def forward(self, input):
        ...
        # print(result.shape)
        result_ = result.reshape(result.shape[0], dataShape[1], -1, result.shape[3], result.shape[4])
...
class _generate_spike(torch.nn.Module):
    def __init__(self,theta=0.9999,T = 5):
        super(_generate_spike, self).__init__()
        self.theta=theta
        self.T=T
        
    def forward(self, input):
        spikes_data = [self.input for _ in range(self.T)]
