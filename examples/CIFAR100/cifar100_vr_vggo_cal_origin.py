...
timestep_f = 200
#VGG_5_
file_load_n = "../../pretrain_weight/cifar100/cifar100_NIPS_t_origin_32_40_.pt"
...
class NewSpike(nn.Module):
    ...
    def forward(self, x):
        x = catCuda.getSpikes(x, T_reduce/timestep-0.001)
...
class AddQuantization_new(object):
    ...
    def __call__(self, tensor):
        x_origin = torch.clamp(torch.div(torch.floor(torch.mul(tensor, timestep)), timestep),min=min_1, max=max_1).cuda()
        #0/10,1/10,2/10,3/10,4/10,5/10,6/10,7/10,8/10

        x_origin_plus_1 = torch.clamp(torch.div(torch.floor(torch.mul(tensor, timestep+1)), timestep+1),min=min_1, max=T_reduce/(timestep+1)).cuda()
        #0/11,1/11,2/11,3/11,4/11,5/11,6/11,7/11,8/11

        my_ones = torch.ones(x_origin_plus_1.shape[0],x_origin_plus_1.shape[1],x_origin_plus_1.shape[2]).cuda()
        for i in range(1,T_reduce+1):
            x_origin_plus_1 = torch.where(x_origin_plus_1 == i/ (  (timestep + 1)), i*my_ones  / timestep, x_origin_plus_1).cuda()

        x_origin_minus_1 = torch.clamp(torch.div(torch.floor(torch.mul(tensor, timestep-1)), timestep-1),min=min_1, max=T_reduce/(timestep-1)).cuda()
        my_ones = torch.ones(x_origin_minus_1.shape[0],x_origin_minus_1.shape[1],x_origin_minus_1.shape[2]).cuda()
        for i in range(1,T_reduce+1):
            x_origin_minus_1 = torch.where(x_origin_minus_1 == i / ((timestep - 1)), i*my_ones   / timestep, x_origin_minus_1).cuda()
    
        x = torch.cat((x_origin, x_origin_plus_1,x_origin_minus_1,x_origin,x_origin_plus_1,x_origin_minus_1), 0).cuda()
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if correct>correct_:
            correct_ = correct
            torch.save(model.state_dict(),"cifar100_IF_200.pt")
        scheduler.step()
