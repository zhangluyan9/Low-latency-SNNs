...
timestep_f = 20
#VGG_5_
file_load_n = "cifar100_NIPS_t16_20.pt"
...
def main():
    ...
    #    print(param_tensor, "\t", model_.state_dict()[param_tensor].size())
    #for param_tensor in model.state_dict():
    #    print(param_tensor, "\t", model.state_dict()[param_tensor].size())
    """
    src_dict = model_.state_dict()
    dst_dict = model.state_dict()
    reshape_dict = {}
    for (k, v) in src_dict.items():
        if k in dst_dict.keys() and 'features.0' not in k :
        #if k in dst_dict.keys()  :
            #print(k)
            if 'num_batches_tracked' in k:
                reshape_dict[k] = nn.Parameter(v.reshape(dst_dict[k].shape),requires_grad=False)
            else:
                reshape_dict[k] = nn.Parameter(v.reshape(dst_dict[k].shape))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if correct>correct_:
            correct_ = correct
            torch.save(model.state_dict(), "cifar100_NIPS_t16_20.pt")
        scheduler.step()
