...
from utils import load_checkpoint, save_checkpoint, get_grad_norm, auto_resume_helper, reduce_tensor, save_latest, update_model_ema, unwrap_model, load_snn_weights # load_weights # load_weights # , load_snn_weights
import copy
from train.optimizer import build_optimizer
from repvggplus_snn_m import create_RepVGGplus_by_name
...
def main(config):
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        # print("Training ACC")
        # acc1, acc5, loss = validate(config, data_loader_val, model)
        # logger.info(f"Training accuracy of the network at epoch {epoch}: {acc1:.3f}%")
