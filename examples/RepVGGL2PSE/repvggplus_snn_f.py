...
def get_spike0(x_out,threshold = 1, p=4):
    spike_out = []
...
def conv_bn_relu(in_channels, out_channels, kernel_size, stride, padding, groups=1):
    ...
    result.add_module('conv', snn.conv(inChannels=in_channels, outChannels=out_channels,
                                                kernelSize=kernel_size, stride=stride, padding=padding, groups=groups, bias=True))
...
class RepVGGplusBlock(nn.Module):
    ...
    def forward(self, x):
        if self.deploy:
            ...
            x = (torch.sum(x, dim=4))/T_reduce
            x = create_spike_input_cuda(x, T_reduce)
            # step2
    ...
    def switch_to_deploy(self):
        ...
        self.rbr_reparm = snn.conv(inChannels=self.rbr_dense.conv.in_channels,
                                     outChannels=self.rbr_dense.conv.out_channels,
                                     kernelSize=self.rbr_dense.conv.kernel_size, stride=self.rbr_dense.conv.stride,
                                     padding=self.rbr_dense.conv.padding, dilation=self.rbr_dense.conv.dilation,
                                     groups=self.rbr_dense.conv.groups, bias=True)
...
class RepVGGplus(nn.Module):
    ...
    def _build_aux_for_stage(self, stage):
        ...
        # ClampModule(min_val=0, max_val=Clp_max), QuantizationModule(T), 
    ...
    def forward(self, x):
        ...
        y = self.gap(out)
        y = (torch.sum(y, dim=4))/T_reduce
        y = create_spike_input_cuda(y, T_reduce)
        # step4
