...
class AddQuantization(object):
    ...
    def __call__(self, tensor):
        ...
        return torch.clamp(torch.div(torch.floor(torch.mul(tensor, T)), T),min=0, max=Clp_max)
...
def build_loader(config):
    ...
    if dataset_val is None:
        ...
        snn_dataset = catSNN.SpikeDataset(dataset_val, T = T*Clp_max, theta=Clp_max - 0.001)
        data_loader_snn = torch.utils.data.DataLoader(
            snn_dataset, sampler=sampler_val,
    ...
    return dataset_train, snn_dataset, data_loader_train, data_loader_snn, mixup_fn
    ...
    if config.DATA.DATASET == 'imagenet':
        ...
        if is_train:
            ...
            for i in range(30):
                ...
                dataset = dataset + datasets.CIFAR10(root='./data', train=True, download=True, transform=im_aug)
        else:
            transform = transforms.Compose(
                [transforms.ToTensor(),
                 # transforms.Normalize(mean, std),
    ...
    if is_train:
        ...
        if config.AUG.PRESET is None:
            ...
            if not resize_im:
                ...
                transform.transforms[1] = AddQuantization()
            else:
                # pass
                transform.transforms[0] = AddQuantization()
                pass
        ...
        # transform.append(AddQuantization())
        return transform
