...
class CatNet(nn.Module):
    ...
    def forward(self, x):
        ...
        y_ = self.snn.spike(self.conv2(x), theta = max_1-0.0001)
        y = self.snn.spike(self.conv3(y_), theta = max_1-0.0001)
        x = self.snn.spike(self.conv4(y), theta = max_1-0.0001)
...
def main():
    ...
    #model.load_state_dict(torch.load("fash_test_q.pt"), strict=False)
