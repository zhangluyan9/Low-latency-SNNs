...
def create_spike_input_cuda(input,T):
    ...
    out = catCuda.getSpikes(input, T_reduce/timestep-0.001)
    return out
...
#9221
    ...
    snn_dataset = SpikeDataset(testset, T = args.T,theta = 0.9*(max_1-0.001))
    ...
    model.load_state_dict(torch.load("cifar10_t300.pt"), strict=False)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if correct>correct_:
            correct_ = correct
            torch.save(model.state_dict(), "cifar10_t300.pt")
