...
def threshold_training_snn(input, y_origin,threshold,T,index):
    ...
    for i in range (y.shape[1]):
        ...
        threshold_1[i] = threshold_1[i] - 0.01*diff*max_1
...
def test_snn(model, device, test_loader):
    ...
    with torch.no_grad():
        ...
        for data, target in test_loader:
            ...
            correct += pred.eq(target.view_as(pred)).sum().item()
            print(pred.eq(target.view_as(pred)).sum().item())
            np.savez(f_store_,threshold_pre_1=threshold_pre_1,threshold_pre_2=threshold_pre_2,threshold_pre_3=threshold_pre_3,threshold_pre_4=threshold_pre_4,threshold_pre_5=threshold_pre_5,threshold_pre_6=threshold_pre_6,threshold_pre_7=threshold_pre_7,p1=p1,p2=p2,p3=p3,p4=p4)
...
#9221
    ...
    snn_dataset = SpikeDataset(trainset, T = args.T,theta = max_1-0.0001)
